...
![Early PCB copper and placement audit](<assets/front-copper-audit.png>)
...
![Intermediate component placement](<assets/revision-e-top.png>)
...
![First JLCPCB quote breakdown](<assets/jlc-price-quote.png>)
...
![JLC stock and alternate-part review](<assets/jlc-stock-review.png>)
...
![OLED header model that had to be removed](<assets/oled-header-review.png>)
...
![Cost before adding the final ESP32 choice](<assets/jlc-cost-before-esp32.png>)
...
![Middle-region placement review](<assets/middle-layout-review.png>)
...
![Revision before the final mechanical cleanup](<assets/revision-e-bottom.png>)
...
![Completed Revision E top view](<assets/revision-e-top.png>)
...
![Top-corner and mounting-hole review](<assets/mounting-layout-review.png>)
...
![JLC preview showing the incorrect ESP32 rotation](<assets/jlc-esp32-orientation-error.png>)
...
![Saved back-side branding and routing](<assets/back-branding-layout.png>)
...
![Final top-side PCB after electrical audit](<assets/final-top.png>)
...
![Final isometric PCB render](<assets/final-isometric.png>)
...
I reorganized the public-facing documentation around Hack Club's shipping checklist: a root readme with motivation, usage, design views, build steps and BOM; a root CSV BOM; an assembly guide; a submission checklist; and this dated image-backed journal. Historical working documents remain local and ignored, while the journal's required images are tracked under `assets/`.
...
![Final PCB back and silkscreen](<assets/final-bottom.png>)
...
I renamed the public root files to `readme.md`, `journal.md`, and `bom.csv`, then updated the release generator and internal links. The bulky local `docs/` history and `Schematics/Legacy/` checkpoint were added to the local ignore configuration and removed only from Git's index, not from the computer. Since ignoring `docs/` would otherwise break the journal images on GitHub, the submission-relevant PNG files were copied into the tracked `assets/` folder.
