...
The current design is the **Final Draft (2026-08-27)** of the Revision E WROOM + CH340C + Ra-01SH board. The exact pre-audit Revision E state is preserved in `../Legacy/Revision E pre-final audit 2026-08-27/`. An S3 radio-board variant was discussed but intentionally not built yet.
...
- Final-audit corrections: CH340C uses switched 3V3 logic, and R26 provides the ICS-43432's recommended 100 kΩ SD pull-down.
...
Final machine checks on 2026-08-27:
...
The Final Draft was machine checked on 2026-08-27: ERC 0 errors and 0 warnings, DRC 0 violations, and 0 unconnected pads.
...
- `ORDER_READINESS_REPORT.md` — final pre-production audit, known risks, and uploader checks.
- `ASSEMBLY_ORIENTATION_CHECK.md` — required human verification of U6/U8 and other critical rotations.
- `FIRST_POWER_UP_CHECKLIST.md` — current-limited prototype bring-up sequence.
- `legacy/revision_c_s3_2026-08-24/` — preserved earlier design.

## Before ordering

Upload the Gerber ZIP, BOM, and **corrected JLC CPL** from `../Final Draft/Manufacturing/` together. Never upload the raw KiCad position CSV as the CPL. In JLC's preview, U6's PCB antenna must face the left edge and U8 must match the supplied top render, with its I-PEX connector on the module's board-interior/lower side. Recheck every Extended row for live stock and exact package match, especially the ICS-43432, TPS63021, CH340C, ESP32-WROOM, MAX98357A, and Ra-01SH.
