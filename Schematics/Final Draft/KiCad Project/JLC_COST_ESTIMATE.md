# JLCPCB cost estimate — Final Draft WROOM + radio

Estimate date: **2026-08-27**. Board: **45 × 100 mm, two layers**. Quantity basis: five bare PCBs with two assembled boards. Shipping, tax, battery, 915 MHz antenna/pigtail, commercial OLED, speaker, enclosure, buttons, potentiometer, external LED, and external power switch are excluded.
...
The remaining charger, USB-C, protection, ESD, fuse, inductor, transistors, LEDs, and passives add parts cost but generally much less per item. Extended feeder/setup charges, not those small unit prices, are the main reason the total rises. The final-audit R26 is Basic and adds only a negligible resistor cost; it does not add an Extended feeder line.

## Planning range

A conservative planning range for **two fully populated Final Draft Economic PCBAs plus five bare boards is approximately $95–$115 before shipping and tax**. This is not a binding quote and may overstate promotional pricing. It extrapolates from the user's $69.84 partial quote and adds the WROOM, Ra-01SH, CH340C path, their support parts, and likely Extended-line fees.
